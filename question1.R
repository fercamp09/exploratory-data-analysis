...
filtered <- pm25 %>% filter( year == 1999 | year == 2002 | year == 2005 | year == 2008 )
grouped_by_year <- filtered %>% group_by(year) %>% summarise(total_emissions = sum(Emissions))

# Graph total emissions per year
par(mfcol = c(1,1))
plot(grouped_by_year$year, grouped_by_year$total_emissions, xlab = "Years", ylab = "Total Emissions (tons)", main = "PM2.5 emissions through the years in US", type = "b")
